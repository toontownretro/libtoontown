...
static void CropString(std::string &S) {
...
std::string Settings::get_config_path(void) {
  ...
  // logic here.
  static std::string ret = "";  // lets please only do this once
  ...
  bool cpth = false;
  std::string cpath;
  std::string cc = ExecutionEnvironment::get_environment_variable("CONFIG_CONFIG");
  if ((!cc.empty()) && (cc.length() > 1)) {
    std::string configconfig(cc);
    std::string assign = "=";
    std::string sep = configconfig.substr(0, 1);
    typedef pvector<std::string> strvec;
    typedef Serialize::Deserializer<strvec, Serialize::StdExt<std::string> > deser;
    ...
    for (strvec::iterator i=sv.begin(); i!=sv.end(); ++i) {
      ...
      if (j != std::string::npos) {
        std::string tok = (*i).substr(0, j);
        std::string rest = (*i).substr(j+1, std::string::npos);
        if (tok == "configpath") {
          if (cpth)
            cpath += " " + rest;
          else
...
#define CONFIG_PATH "."
    ...
    while (!cpath.empty()) {
      ...
      std::string stmp = cpath.substr(0, i);
...
inline void write_nibble(std::ostream &os, unsigned char n) {
  switch (n) {
  case 0:
    std::cerr << "0";
    break;
...
inline void write_byte(std::ostream &os, unsigned char b) {
...
void Settings::ns_write_settings(void) {
  ...
  std::string path = get_config_path();
  DSearchPath setting_search(path);
  std::string dir = setting_search.get_directory(0);
  ...
  if (canWrite == 0){
      std::cerr << "could not open '" << _fname << "' or '" << f2 << "' for writing"
           << std::endl;
      return;
  }
...
set_resolution_dimensions(unsigned int xsize,unsigned int ysize) {
  ...
  if(r>=R_NONE) {
      std::cerr << "set_res failed, Enum Resolution does not contain " << xsize << "x" << ysize << std::endl;
      return;
  }
