...
//       Access: Public
//  Description: Store a suit edge represented by the start and std::endl
...
//       Access: Public
//  Description: Return the number of entries in this catalog
//               Return empty std::string if the catalog is not found
////////////////////////////////////////////////////////////////////
...
int DNAStorage::remove_DNAGroup(PT(PandaNode) rr) {
  ...
  if (group == (DNAGroup *)NULL) {
    dna_cat.warning()
      << "Rstd::endler relation did not point to any DNAGroups in the storage" << std::endl;
    return 0;
  } else {
...
PT(DNASuitEdge) DNAStorage::get_suit_edge(int start_index, int end_index) const {
  ...
  if (i == _suit_start_point_map.end()) {
    ...
    // the std::endl index. Find the std::endl index in the edge vector associated
    ...
    // Did not find the std::endl point connected to this start point
    dna_cat.error()
      << "DNASuitStartPoint start index: " << start_index
      << " not connected to std::endl index: " << end_index << std::endl;
    return (DNASuitEdge *)NULL;
  }
}
...
//               from the start point to the std::endl point if he is
...
//       Access: Public
//  Description: Find a valid path from start to std::endl for a suit to
      ...
      for(SuitEdgeVector::const_iterator evi = edge_list.begin();
        ...
        if (!edge->get_end_point()->is_terminal()) {
          if (next_point_index == current_point_index || 
            ...
            // Extstd::endl the path by the current point and save it on the
            // new chain.
...
//     Function: get_block
//       Access: Public
//  Description: Get the block number as a std::string from the building
//               name.
////////////////////////////////////////////////////////////////////
